 ...
 * نفس آلية استعادة كلمة المرور لضمان وصول الرسالة
 * @param string $to - البريد الإلكتروني للمستلم
...
function sendVerificationEmail($to, $verifyLink, $userName = '') {
    $appName = function_exists('env') ? (env('APP_NAME', '') ?: env('MAIL_FROM_NAME', 'AmrNayl Academy')) : 'AmrNayl Academy';
    // موضوع مشابه لاستعادة كلمة المرور لتحسين وصول الرسالة (تجنب التصفية)
    $subject = 'تفعيل حسابك - ' . $appName;

    $safeName = $userName !== '' ? htmlspecialchars($userName, ENT_QUOTES, 'UTF-8') : '';
    $greeting = $safeName !== '' ? 'مرحباً ' . $safeName . '،' : 'مرحباً،';
...
<head>
    ...
    <style>
        ...
        .link-note { font-size: 0.85rem; color: #8b95a5; word-break: break-all; }
        .footer { margin-top: 25px; padding-top: 15px; border-top: 1px solid #eee; font-size: 0.85rem; color: #8b95a5; }
        .spam-note { background: #fff8e6; padding: 10px; border-radius: 8px; font-size: 0.9rem; margin: 10px 0; }
    </style>
</head>
<body>
    <div class="container-app">
        <div class="logo">' . htmlspecialchars($appName, ENT_QUOTES, 'UTF-8') . '</div>
        <h1>تفعيل حسابك</h1>
        <p>' . $greeting . '</p>
        <p>شكراً لتسجيلك. انقر على الزر أدناه لتأكيد بريدك الإلكتروني وتفعيل حسابك:</p>
        <p><a href="' . $safeLink . '" class="btn">تأكيد البريد الإلكتروني</a></p>
        <p class="link-note">إن لم يعمل الزر، انسخ الرابط التالي إلى المتصفح:<br>' . $safeLink . '</p>
        <p>رابط التأكيد صالح لمدة 24 ساعة.</p>
        <p class="spam-note">إن لم تجد الرسالة في الوارد، تحقق من مجلد <strong>الرسائل غير المرغوبة</strong> (Spam).</p>
...
function sendEmailViaSmtp($to, $subject, $bodyHtml) {
    ...
    if (!is_resource($socket)) {
        error_log("SMTP connection failed: {$addr} (errno {$errno}) {$errstr}");
        return false;
    }
    ...
    $read = function () use ($socket, &$lastResponse) {
        ...
        $lastResponse = $line === false ? '' : trim($line);
        return $lastResponse;
    };
    $expect = function ($code) use ($socket, $read, &$lastResponse) {
        $line = $read();
        $lastResponse = $line;
        $got = substr($line, 0, 3);
    ...
    $failLog = function ($step) use (&$lastResponse) {
        error_log("SMTP [{$step}]: " . ($lastResponse !== '' ? $lastResponse : 'no response'));
    };
    ...
    if (!$useSsl && ($port === 587 || $encryption === 'tls')) {
        ...
        if (!@stream_socket_enable_crypto($socket, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
            error_log('SMTP [STARTTLS]: stream_socket_enable_crypto failed');
            fclose($socket);
            return false;
        }
        $send('EHLO ' . ($_SERVER['SERVER_NAME'] ?? 'localhost'));
        if (!$expect(250)) {
            $failLog('EHLO-after-STARTTLS');
            fclose($socket);
            return false;
        }
    }
    ...
    if (!$expect(334)) {
        $failLog('AUTH-username');
        fclose($socket);
        return false;
    }
    $send(base64_encode($password));
    if (!$expect(235)) {
        $failLog('AUTH-password');
        fclose($socket);
        return false;
    }
    ...
    if (!@fwrite($socket, $data . "\r\n")) {
        error_log('SMTP [DATA-write]: fwrite failed');
        fclose($socket);
        return false;
    }
    if (!$expect(250)) {
